import { useNavigate } from "react-router-dom";
import CommonButton from "../commonButton";

function AddedToCart({ forWishList }) {
  const navigate = useNavigate();
  return (
    <div
      onClick={(e) => {
        e.stopPropagation();
      }}
      className="font-montserrat w-full mt-5"
    >
      <CommonButton
        btnText={
          <span
            className={`flex gap-2 items-center ${
              forWishList ? "sm:text-xl text-md" : "text-xl"
            } font-montserrat uppercase mx-auto`}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 -960 960 960"
              width="30px"
              height="30px"
              className={`fill-white ${
                forWishList
                  ? "sm:w-[30px] w-[20px] sm:h-[30px] h-[20px]"
                  : "w-[30px] h-[30px]"
              }`}
            >
              <path d="M280-80q-33 0-56.5-23.5T200-160q0-33 23.5-56.5T280-240q33 0 56.5 23.5T360-160q0 33-23.5 56.5T280-80Zm400 0q-33 0-56.5-23.5T600-160q0-33 23.5-56.5T680-240q33 0 56.5 23.5T760-160q0 33-23.5 56.5T680-80ZM246-720l96 200h280l110-200H246Zm-38-80h590q23 0 35 20.5t1 41.5L692-482q-11 20-29.5 31T622-440H324l-44 80h480v80H280q-45 0-68-39.5t-2-78.5l54-98-144-304H40v-80h130l38 80Zm134 280h280-280Z"></path>
            </svg>
            In Cart
          </span>
        }
        className={
          "w-full rounded-lg bg-coral-red flex justufy-center text-white py-1 standard-btn-hover"
        }
        btnTitle={"Added to cart"}
        handleOnClick={() => navigate("/cart")}
      />
    </div>
  );
}

export default AddedToCart;
